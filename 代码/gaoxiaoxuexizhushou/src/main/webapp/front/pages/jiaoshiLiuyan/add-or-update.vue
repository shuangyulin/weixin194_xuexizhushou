    ...
    <view class="content">
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"#008761","borderRadius":"0px","color":"#008761","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"280rpx"}'
                        v-model='ruleForm.jiaoshiLiuyanText' placeholder="评论内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`jiaoshiLiuyan`, this.ruleForm.id);
        ...
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.jiaoshiLiuyanText)) {
                this.$utils.msg(`评论内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`jiaoshiLiuyan`, this.ruleForm);
                } else {
                    await this.$api.save(`jiaoshiLiuyan`, this.ruleForm);
                }
