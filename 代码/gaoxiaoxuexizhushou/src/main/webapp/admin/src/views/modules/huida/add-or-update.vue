    ...
    <div class="addEdit-block">
        <el-form
            ...
            <el-row>
                ...
                <el-col :span="12" v-if="sessionTable !='zuoye' ">
                    <el-form-item class="upload" v-if="type!='info' && !ro.zuoyePhoto" label="封面" prop="zuoyePhoto">
                        <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in (zuoyeForm.zuoyePhoto || '').split(',')" :src="item" width="100" height="100">
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.zuoyePhoto" label="封面" prop="zuoyePhoto">
                            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in (ruleForm.zuoyePhoto || '').split(',')" :src="item" width="100" height="100">
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="12"  v-if="sessionTable !='zuoye' ">
                ...
                <el-col :span="12">
                    ...
                    <div v-else>
                        <el-form-item v-if="ruleForm.huifuFile" label="回答文件" prop="huifuFile">
                            <a type="text" style="text-decoration:none" class="el-button" :href="ruleForm.huifuFile"  >回答文件下载</a>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="批改状态" prop="huidaTypes">
                        <el-select v-model="ruleForm.huidaTypes" :disabled="ro.huidaTypes" placeholder="请选择批改状态">
                            <el-option
                                v-for="(item,index) in huidaTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="批改状态" prop="huidaValue">
                        <el-input v-model="ruleForm.huidaValue"
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                   ...
                   huidaTypes: [
                              { required: true, message: '批改状态不能为空', trigger: 'blur' },
                              {  pattern: /^[0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=huida_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.huidaTypesOptions = data.data.list;
                    }
                });
